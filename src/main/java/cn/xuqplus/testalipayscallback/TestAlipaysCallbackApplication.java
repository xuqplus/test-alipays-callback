...
public class TestAlipaysCallbackApplication {
  ...
  @GetMapping("order")
  public Object order(String no, String amount, String memo, ModelAndView mav, HttpServletRequest request) throws Exception {
    LOGGER.info("order no={}, amount={}, memo={}", no, amount, memo);
    if (StringUtils.isEmpty(no)) {
      ...
      orders.put(no, new Object[]{no, amount, memo, createAt, merchant, null, null, null});
  ...
  @GetMapping("notify")
  public Object notify(String no, String aliTradeNo) {
    LOGGER.info("notify no={}, aliTradeNo={}", no, aliTradeNo);
    orders.get(no)[7] = true;
    return new Res("0", "ok", null);
  }
}
